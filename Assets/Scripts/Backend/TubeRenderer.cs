...
public class TubeRenderer : MonoBehaviour
{
    ...
    private Vector3[] CalculateCircle(int index)
    {
        ...
        if (index != 0)
        {
            currentLength += Vector3.Distance(_positions[index], _positions[index - 1]) / 2;
        }

        //Debug.Log("total: " + totalLength + " | now: " + currentLength);
        float t = ((currentLength / totalLength) * 1); // number between 1 and 0;
